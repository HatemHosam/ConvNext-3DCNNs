...
def convnext_tiny_2d(pretrained=True,in_22k=False, **kwargs):
...
def copy_weights_2d_to_3d(C2D_model, C3D_model):
    ...
    for i in range(3):
        ...
        C3D_model.downsample_layers[i][0].weight.data = C2D_model.downsample_layers[i][0].weight.data.unsqueeze(2).repeat(1, 1, C3D_model.downsample_layers[i][0].weight.shape[2], 1, 1)
